...
def seleccionar_contorn_skittle(contornos, shape):
    ...
    for contorno in contornos:
        ...
        marge = 5
        toca_vora = (
            x <= marge or y <= marge or x + w >= ancho - marge or y + h >= alto - marge
        )
        if toca_vora:
            continue
...
def detectar_color_skittle_desde_array(imagen, mostrar=True):
    ...
    if mostrar:
        ...
        texto = f"{color}"
        cv2.putText(vis, texto, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        cv2.imshow('Imatge detectada', vis)
        cv2.waitKey(0)
...
def main():
    parser = argparse.ArgumentParser(description='Captura amb rpicam-still --immediate i detecta el color del skittle sense guardar la foto')
